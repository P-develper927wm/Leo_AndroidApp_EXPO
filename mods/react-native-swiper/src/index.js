...
export default class extends Component {
   ...
   */
    ...
    //hack: current index is stored in state, so adding or removing children throws things off.
    // Temporary hack is to add another parameter to bypass the state index for now

  scrollBy = (index, animated = true, rulesHack = false) => {
    if (this.internals.isScrolling || this.state.total < 2) return
    const state = this.state
    //Original line below
    //const diff = (this.props.loop ? 1 : 0) + index + this.state.index
    const diff = ! rulesHack ? (this.props.loop ? 1 : 0) + index + this.state.index : 2
  ...
  render () {
    ...
    //HACK: need to remove nulls from children for conditional rendering
    // For make infinite at least total > 1
    if (total > 1) {

      let newChildren = {}
      //remove nulls and false from children to enable conditional rendering
      for (key in children)
        children[key] && (newChildren[key] = children[key])

      // Re-design a loop model for avoid img flickering
      pages = Object.keys(newChildren)
      if (loop) {
      ...
      pages = pages.map((page, i) => {
        if (loadMinimal) {
          if (i >= (index + loopVal - loadMinimalSize) &&
            ...
            return <View style={pageStyle} key={i}>{newChildren[page]}</View>
          } else {
            return (
      ...
      pages = <View style={pageStyle} key={0}>{newChildren}</View>
    }
